"""
Demo 3: Interactive Agent with Memory Tools — Chatbot.

Talks to the deployed memory-tools agent, demonstrating that memory
persists across conversation turns. The agent uses explicit memory tools
(remember, recall) to store and retrieve information.

Try telling the agent your preferences, then ask about them later to see
memory in action.

Usage:
    python invoke_agent.py                           # Interactive chatbot
    python invoke_agent.py "Remember: I prefer dark mode"  # Single prompt
"""
...
def invoke(runtime_arn, prompt, region, session_id=None):
    """Send a prompt to the deployed memory-tools agent."""
...
def run_chatbot(runtime_arn, region, session_id):
    """Interactive conversation loop demonstrating memory persistence."""
    print(f"\n{BOLD}{WHITE}  Memory Tools Agent Chat{RESET}")
    print(f"  The agent remembers things you tell it across turns.")
    print(f"  Try: 'Remember that I like sushi' then later 'What food do I like?'")
...
def main():
    ...
    info("Memory persists across turns — the agent uses explicit tools to remember/recall")
