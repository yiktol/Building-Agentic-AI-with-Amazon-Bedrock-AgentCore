"""
Demo 2: Interactive Chat Client — Outbound Auth (API Key).

Chat client for the deployed agent with API key outbound auth.
The agent retrieves its API key from the token vault transparently.

Usage:
    python invoke_agent.py                              # Interactive
    python invoke_agent.py "Look up AI news"            # Single prompt
"""
...
def run_chatbot(runtime_arn, region, session_id):
    print(f"\n{BOLD}{WHITE}  API Key Agent Chat{RESET}")
    print(f"  Agent uses @requires_api_key to access external APIs.")
    print(f"  Key is stored in Secrets Manager — never in LLM context.")
...
def main():
    ...
    banner("Demo 2: API Key Agent (Interactive)")
    config_val("Runtime ARN", runtime_arn)
    config_val("Session", session_id)
    info("Agent retrieves API key from vault via @requires_api_key")
