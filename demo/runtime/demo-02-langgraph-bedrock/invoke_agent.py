"""
Demo 2: Interactive Agent Chatbot — LangGraph + Bedrock on AgentCore Runtime.

Chat client for the deployed LangGraph agent (math + weather tools).
Same session ID maintained throughout → agent remembers context.

Usage:
    python invoke_agent.py                          # Interactive chatbot
    python invoke_agent.py "Calculate sqrt(144)"    # Single prompt
"""
...
def invoke(runtime_arn, prompt, region, session_id):
    """Send a prompt to the deployed LangGraph agent."""
...
def run_chatbot(runtime_arn, region, session_id):
    ...
    print(f"\n{BOLD}{WHITE}  LangGraph Agent Chat{RESET}")
    print(f"  Tools: calculator, get_weather")
    print(f"  Same session — agent remembers context across turns.")
...
def main():
    ...
    banner("Demo 2: LangGraph + Bedrock Agent (Interactive)")
    config_val("Runtime ARN", runtime_arn)
    config_val("Session", session_id)
    info("Same deployment API — different framework inside")
