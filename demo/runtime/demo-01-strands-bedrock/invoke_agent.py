"""
Demo 1: Interactive Agent Chatbot — Strands + Bedrock on AgentCore Runtime.

Connects to the deployed agent (weather + calculator tools) and provides
an interactive chat loop. Each message is sent to the runtime endpoint.

Usage:
    python invoke_agent.py                     # Interactive chatbot
    python invoke_agent.py "What's 25 * 17?"   # Single prompt
"""
...
def invoke(runtime_arn, prompt, region, session_id=None):
    """Send a prompt to the deployed agent runtime."""
...
def run_chatbot(runtime_arn, region, session_id):
    """Interactive conversation loop with the deployed agent."""
    print(f"\n{BOLD}{WHITE}  Interactive Agent Chat{RESET}")
...
def main():
    ...
    banner("Demo 1: Strands + Bedrock Agent (Interactive)")
